...
JSONL records are {"text": <string>}; the legacy JSON array contains nonblank strings.
The rollback file is notes.json.bak, preserving the original legacy bytes. A different
existing backup must never be overwritten. Invalid legacy input or a backup conflict
must exit nonzero without modifying either store. List prints each note on its own
line. Commands run from the directory containing the data. No external services.
